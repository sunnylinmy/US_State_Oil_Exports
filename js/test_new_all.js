query_str_import = "http://api.census.gov/data/2014/intltrade/stnaics?get=VALUE_YR,STATE&YEAR=2014&NAICS=3241&CTY_CODE";


function val(d) {return d.VALUE_YR;};
	...
	num_data = data.splice(1,data.length-1);
		...
		for (i=0;i<stateName.length;i++){
			...
			for (j=0;j<states_json.length;j++){
				sumByState = sumByState*1+Number(states_json[j].VALUE_YR);
			}
...
state_svg.on("click", function(){
  ...
  var bar = barchart.selectAll(".bar")
            ...
            .style("width", function(d) {return xLength(d.VALUE_YR) + "px";})
            .style("height", 25+"px")
            .append("text")
            .text(function(d){
